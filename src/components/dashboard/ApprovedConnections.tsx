...
export function ConnectionRequests({ profile }: { profile: Profile }) {
  ...
  const respond = async (request: ConnectionRequest, status: 'accepted' | 'rejected') => { const { error } = await supabase.from('connection_requests').update({ status }).eq('id', request.id); if (!error) setRequests((current) => current.filter((item) => item.id !== request.id)); };
...
export function BankDonorFinder({ profile }: { profile: Profile }) {
  const [donors, setDonors] = useState<(Donor & { profiles?: Profile })[]>([]); const [query, setQuery] = useState(''); const [group, setGroup] = useState<BloodGroup>('O+');
  useEffect(() => { (async () => { const { data } = await supabase.from('donors').select('*, profiles(*)').eq('availability_status', 'available').eq('blood_group', group); setDonors((data as (Donor & { profiles?: Profile })[]) || []); })(); }, [group]);
  const send = async (donor: Donor) => { await supabase.from('connection_requests').insert({ kind: 'bank_donor', requester_id: profile.id, recipient_id: donor.user_id, blood_group: donor.blood_group, units: 1, message: 'Your blood group matches our current blood requirement.' }); };
  ...
  return <Card><CardHeader title="Find Matched Donors" subtitle="Send a request; chat opens only after the donor accepts." icon={<Search className="h-5 w-5" />} /><div className="p-5"><div className="grid gap-3 sm:grid-cols-2"><Select label="Required blood group" value={group} onChange={(event) => setGroup(event.target.value as BloodGroup)}>{BLOOD_GROUPS.map((item) => <option key={item}>{item}</option>)}</Select><Input label="Search donor or city" value={query} onChange={(event) => setQuery(event.target.value)} /></div><div className="mt-5 space-y-3">{visible.map((donor) => <div key={donor.id} className="flex items-center gap-3 rounded-xl border border-slate-200 p-4"><BloodGroupBadge group={donor.blood_group} /><div className="flex-1"><p className="text-sm font-semibold text-slate-900">{donor.profiles?.full_name || 'Donor'}</p><p className="text-xs text-slate-500">{donor.profiles?.city || 'Location not provided'}</p></div><Button size="sm" onClick={() => void send(donor)} icon={<Send className="h-4 w-4" />}>Request Blood</Button></div>)}{visible.length === 0 && <EmptyState icon={<Search className="h-6 w-6" />} title="No available donors" description="Try another blood group or location." />}</div></div></Card>;
}
