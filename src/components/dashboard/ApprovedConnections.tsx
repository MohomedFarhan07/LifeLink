...
export function BankDonorFinder({ profile }: { profile: Profile }) {
  ...
  return <Card><CardHeader title="Find Matched Donors" subtitle="Send a request; chat opens only after the donor accepts." icon={<Search className="h-5 w-5" />} /><div className="p-4 sm:p-5"><div className="grid gap-3 sm:grid-cols-2"><Select label="Required blood group" value={group} onChange={(event) => setGroup(event.target.value as BloodGroup)}>{BLOOD_GROUPS.map((item) => <option key={item}>{item}</option>)}</Select><Input label="Search donor or city" value={query} onChange={(event) => setQuery(event.target.value)} /></div><div className="mt-5 space-y-3">{visible.map((donor) => { const requested = requestedDonorIds.has(donor.user_id); return <div key={donor.id} className="flex flex-col gap-3 rounded-xl border border-slate-200 p-4 sm:flex-row sm:items-center"><BloodGroupBadge group={donor.blood_group} /><div className="min-w-0 flex-1"><PublicProfileLink userId={donor.user_id} role="donor" label={donor.profiles?.full_name || 'Donor'} className="text-sm font-semibold text-slate-900" /><p className="text-xs text-slate-500">{donor.profiles?.city || 'Location not provided'}</p></div><Button size="sm" disabled={requested} variant={requested ? 'outline' : 'primary'} onClick={() => void send(donor)} icon={<Send className="h-4 w-4" />}>{requested ? 'Request sent' : 'Request Blood'}</Button></div>; })}{visible.length === 0 && <EmptyState icon={<Search className="h-6 w-6" />} title="No available donors" description="Try another blood group or location." />}</div></div></Card>;
}
