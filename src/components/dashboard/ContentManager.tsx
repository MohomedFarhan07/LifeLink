...
export function ContentManager({ role }: { role: 'blood_bank' | 'hospital' }) {
  ...
  const openEditor = (type: ContentType, item?: Awareness | SuccessStory | Campaign) => {
    ...
    if (type === 'awareness') { const a = item as Awareness; setForm({ ...base, content: a.content, category: a.category }); }
    else if (type === 'story') { const s = item as SuccessStory; setForm({ ...base, recipient_name: s.recipient_name, story_date: s.story_date }); }
  ...
  const save = async () => {
    ...
    const values = type === 'awareness'
      ? { title: form.title, description: form.description, content: form.content, category: form.category || 'general', image_url: form.image_url, author_id: profile.id, author_name: profile.full_name }
      : type === 'story'
        ? { title: form.title, description: form.description, image_url: form.image_url, recipient_name: form.recipient_name, story_date: form.story_date, author_id: profile.id, author_name: profile.full_name }
