...
export function ImageDropzone({ value, contentType, onChange }: { value: string; contentType: 'awareness' | 'story' | 'campaign'; onChange: (url: string) => void }) {
  ...
  const previewStyle: CSSProperties = {
    ...
    aspectRatio: crop === 'square' ? '1 / 1' : crop === 'landscape' ? '16 / 9' : undefined,
    // The original view is for inspecting the complete upload. Crop previews
    // intentionally use cover so users can see the selected output format.
